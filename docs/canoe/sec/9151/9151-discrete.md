...
要求其合取范式，就要找公式真值为假的项，令每个元命题真值均为假，通过析取 ∪ 相耦后再通过合取 ∩ 相连

- 第四行`A->¬B`为假，于是取这一行，令此时的 A、B 均为假，即`¬A`和`¬B`，再通过析取 ∪ 相耦合，得到范式中的一个小项`¬A∪¬B`，由于只存在一个小项，故`¬A∪¬B`就是该式的合取范式
- 求析取范式则要找真值为真的项，可得三个合取小项`¬A∩¬B、¬A∩B、A∩¬B`（令其中每个原子命题真值均为真），最后通过析取 ∪ 相连，得到析取范式`(¬A∩¬B)∪(¬A∩B)∪(A∩¬B)`
...
进一步可推知，存在偶数个结点的欧拉图总边数不可能是奇数，因为边数 = 总度数/2，而总度数一定为两个偶数相乘（有偶数个结点且每个结点均为偶数度结点），故边数一定为偶数
