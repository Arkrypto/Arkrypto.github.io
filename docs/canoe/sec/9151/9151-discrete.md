...
要求其合取范式，就要找公式真值为假的项，令每个元命题真值均为假，再通过合取耦合

第四行`A->¬B`为假，于是取这一行，可知 A、B 均为真，令其为假，即`¬A`和`¬B`，再通过合取耦合，得到范式中的一个小项`¬A∪¬B`

求析取范式则要找真值为真的项，可得三个析取小项`¬A∩¬B、¬A∩B、A∩¬B`，其中每个元命题真值均令其为真，最后通过合取相连，得到析取范式`(¬A∩¬B)∪(¬A∩B)∪(A∩¬B)`
