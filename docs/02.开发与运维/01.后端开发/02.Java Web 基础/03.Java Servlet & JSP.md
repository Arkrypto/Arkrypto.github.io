...
- 数据共享：ServletContext作为一个数据互通的容器，在这个Servlet中保存的数据，可以在另外一个Servlet中获取
...
请求转发：将Study映射页面的请求转发到 /hello，url不发生改变，但页面内容显示 /hello 的内容
...
- 相同点：实现页面的跳转，信息处理的迁移
- 不同点：
  1. 转发是将请求转发到另一地址，重定向是用另一个地址去响应
  2. 重定向url栏会改变，而转发不发生改变
